...
class AuthServiceProxy(object):
    def __init__(self, serviceURL, serviceName = None, numberOfAttempts = 1, httpTimeout = HTTP_TIMEOUT):
        ...
        # print "DEBUG - EFE: self.__numberOfAttempts %s" % self.__numberOfAttempts
    ...
    def __getattr__(self, name):
        ...
        return AuthServiceProxy(self.__serviceURL, name, self.__numberOfAttempts)
    ...
    def __call__(self, *args):
         ...
         attemptCredit = self.__numberOfAttempts
         while not responseReceived:
             try:
                 ...
                 if attemptCredit <= 0:
                     raise Exception('Failed to complete bitcoin RPC.')
                 print(colors.WARNING + "Remaining attempts: "+ str(attemptCredit) +colors.ENDC)
                 sys.stdout.flush()
